...
class BlockedUsersTableViewController: UITableViewController {
    ...
    func fetchBlockedUsers()
    {
        ...
        blockedUsersRef?.observe(.value, with: { (
            ...
            {
                var key = (item as! FIRDataSnapshot).key
                var value = ((item as! FIRDataSnapshot).value) as! String
   ...
   func removeBlockButtonClicked(sender: UIButton!)
    {
        ...
        var row =  sender.tag
          ...
          var blockReference = FIRDatabase.database().reference().child("Users").child(self.userID).child("block").child(blockedUsersKeys[row]).removeValue()
