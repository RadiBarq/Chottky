...
class ChatCollectionViewController: UICollectionViewController, UICollectionViewDelegateFlowLayout {
    ...
    var observeReference: FIRDatabaseReference?
    ...
    {
        var userImageRef = FIRStorage.storage().reference().child("Profile_Pictures").child(ChatCollectionViewController.messageToId).child("Profile.jpg")
        var itemImageRef = FIRStorage.storage().reference().child("Items_Photos").child(ChatCollectionViewController.itemId).child("1.jpeg")
    ...
    func clickMore()
    {
      ...
      FIRDatabase.database().reference().child("Users").child(WelcomeViewController.user.userId).child("chat").child(ChatCollectionViewController.messageToId +  ChatCollectionViewController.itemId).removeValue(completionBlock: { (error, ref) in
   ...
   override func viewWillAppear(_ animated: Bool) {
    ...
    func handleSend()
    {
        ...
        if (messagesTexts != "")
        {
            ...
            let ref = FIRDatabase.database().reference().child("Users").child(userID).child("chat").child(ChatCollectionViewController.messageToId + ChatCollectionViewController.itemId).childByAutoId().updateChildValues(["messageText": inputTextField.text, "messageFrom": WelcomeViewController.user.userId, "timestamp": timestamp])
            
            let ref2 = FIRDatabase.database().reference().child("Users").child(ChatCollectionViewController.messageToId).child("chat").child(userID + ChatCollectionViewController.itemId).childByAutoId().updateChildValues(["messageText": inputTextField.text, "messageFrom": WelcomeViewController.user.userId, "timestamp": timestamp])
            FIRDatabase.database().reference().child("Users").child(ChatCollectionViewController.messageToId).child("chat").child(userID + ChatCollectionViewController.itemId).child("lastMessage").updateChildValues(["message": inputTextField.text, "time": timestamp, "recent": "true"])
            FIRDatabase.database().reference().child("Users").child(userID).child("chat").child(ChatCollectionViewController.messageToId + ChatCollectionViewController.itemId).child("lastMessage").updateChildValues(["message": inputTextField.text, "time": timestamp, "recent": "false"])
                // notificationReference.updateChildValues(["notified": true])
            
            FIRDatabase.database().reference().child("Users").child(ChatCollectionViewController.messageToId).child("chat").child(userID + ChatCollectionViewController.itemId).updateChildValues(["item-id": ChatCollectionViewController.itemId])
            
            FIRDatabase.database().reference().child("Users").child(ChatCollectionViewController.messageToId).child("chat").child(userID + ChatCollectionViewController.itemId).updateChildValues(["user-id": userID])
            
            FIRDatabase.database().reference().child("Users").child(userID).child("chat").child(ChatCollectionViewController.messageToId + ChatCollectionViewController.itemId).updateChildValues(["item-id": ChatCollectionViewController.itemId])
            FIRDatabase.database().reference().child("Users").child(userID).child("chat").child(ChatCollectionViewController.messageToId + ChatCollectionViewController.itemId).updateChildValues(["user-id": ChatCollectionViewController.messageToId])
            
            FIRDatabase.database().reference().child("Users").child(ChatCollectionViewController.messageToId).child("chat").child(userID + ChatCollectionViewController.itemId).updateChildValues(["user-name": userDisplayName])
            
            FIRDatabase.database().reference().child("Users").child(userID).child("chat").child(ChatCollectionViewController.messageToId + ChatCollectionViewController.itemId).updateChildValues(["user-name": ChatCollectionViewController.messageFromDisplayName])
        ...
        //FIRDatabase.database().reference().child("Users").child(userID).child("chat").child(ChatCollectionViewController.messageToId).child("lastMessage").updateChildValues(["recent": "false"])
           ...
           FIRDatabase.database().reference().child("messages").childByAutoId().updateChildValues([
    ...
    {
        observeReference = FIRDatabase.database().reference().child("Users").child(userID).child("chat").child(ChatCollectionViewController.messageToId + ChatCollectionViewController.itemId)
        observeReference?.observe(.value, with: {outerSnapshot in
            
            FIRDatabase.database().reference().child("Users").child(ChatCollectionViewController.messageToId).child("block").observeSingleEvent(of: .value, with:{ (snapshot) in
